...
import (
	...
	"github.com/ribice/homeschooling/pkg/api/auth/transport"
	"github.com/ribice/homeschooling/pkg/utl/jwt"
	authMw "github.com/ribice/homeschooling/pkg/utl/middleware/auth"
...
func TestLogin(t *testing.T) {
	cases := []struct {
		...
		req        string
		wantStatus int
		wantResp   *homeschooling.AuthToken
		udb        *mockdb.User
		jwt        *mock.JWT
		sec        *mock.Secure
	}{
		...
		},
		{
			...
			wantResp: &homeschooling.AuthToken{Token: "jwttokenstring", RefreshToken: "refreshtoken"},
		},
	}
	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantResp != nil {
				response := new(homeschooling.AuthToken)
...
func TestRefresh(t *testing.T) {
	cases := []struct {
		...
		req        string
		wantStatus int
		wantResp   *homeschooling.RefreshToken
		udb        *mockdb.User
		jwt        *mock.JWT
	}{
		{
		...
		},
		{
			...
			wantResp: &homeschooling.RefreshToken{Token: "jwttokenstring"},
		},
	}
	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			...
			defer res.Body.Close()
			if tt.wantResp != nil {
				response := new(homeschooling.RefreshToken)
				if err := json.NewDecoder(res.Body).Decode(response); err != nil {
					t.Fatal(err)
				}
				assert.Equal(t, tt.wantResp, response)
...
func TestMe(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			rbac: &mock.RBAC{
				UserFn: func(echo.Context) homeschooling.AuthUser {
					return homeschooling.AuthUser{ID: 1}
				},
			},
		...
		},
		{
			...
			wantStatus: http.StatusOK,
			udb: &mockdb.User{
				ViewFn: func(db orm.DB, i int) (homeschooling.User, error) {
					return homeschooling.User{
	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			...
			defer res.Body.Close()
			if tt.wantResp.ID != 0 {
				var response homeschooling.User
				if err := json.NewDecoder(res.Body).Decode(&response); err != nil {
					t.Fatal(err)
				}
				assert.Equal(t, tt.wantResp, response)
