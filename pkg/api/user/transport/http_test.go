...
func TestCreate(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			rbac: &mock.RBAC{
				AccountCreateFn: func(c echo.Context, roleID homeschooling.AccessRole, companyID, locationID int) error {
...

		{
			...
			udb: &mockdb.User{
				CreateFn: func(db orm.DB, usr homeschooling.User) (homeschooling.User, error) {
					usr.ID = 1
	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantResp != nil {
				response := new(homeschooling.User)
...
func TestList(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			rbac: &mock.RBAC{
				UserFn: func(c echo.Context) homeschooling.AuthUser {
		...
		},
		{
			...
			udb: &mockdb.User{
				ListFn: func(db orm.DB, q *homeschooling.ListQuery, p homeschooling.Pagination) ([]homeschooling.User, error) {
					...
					return nil, homeschooling.ErrGeneric
				},
			},
			wantStatus: http.StatusOK,
			wantResp: &listResponse{
				Users: []homeschooling.User{
					{
						Base: homeschooling.Base{
							ID:        10,
							CreatedAt: mock.TestTime(2001),
	...
	cases := []struct {
		...
		},
		{
			...
			udb: &mockdb.User{
				...
				UpdateFn: func(db orm.DB, usr homeschooling.User) error {
		...
		},
		{
			...
			udb: &mockdb.User{
				ViewFn: func(db orm.DB, id int) (homeschooling.User, error) {
					return homeschooling.User{
						Role: &homeschooling.Role{
							AccessLevel: homeschooling.CompanyAdminRole,
						},
					}, nil
				},
			},
			rbac: &mock.RBAC{
				IsLowerRoleFn: func(echo.Context, homeschooling.AccessRole) error {
					return echo.ErrForbidden
				},
			},
			wantStatus: http.StatusForbidden,
